...
typedef CustomComputeCallback<Q, R> = FutureOr<R> Function(Q message);

class CustomComputeController<R> {
  const CustomComputeController._(
    this._isolate,
    this.future,
  );

  Future<void> dispose() async {
    return (await _isolate.future).kill(priority: Isolate.immediate);
  }
...
CustomComputeController<R> customCompute<Q, R>(CustomComputeCallback<Q, R> callback, Q message, {String? debugLabel}) {
  debugLabel ??= kReleaseMode ? 'compute' : callback.toString();

  Completer<Isolate> isolate = Completer();
  ...
  try {
    isolate.complete(Isolate.spawn<_IsolateConfiguration<Q, R>>(
      _spawn,
  ...
  final response = completer.future.then((response) async {
    if (response == null) {
  ...
  return CustomComputeController._(isolate, response);
}
...
class _IsolateConfiguration<Q, R> {
  ...
  final CustomComputeCallback<Q, R> callback;
  final Q message;
